...
// form
  ...
  displayedColumns: string[] = ['name', 'quantity', 'vendorid', 'cost', 'ext'];
  ...
  onPickVendor(): void {
    this.subscription = this.generatorForm.get('vendorid').valueChanges.subscribe(val => {
      ...
      this.vendorproducts$.subscribe(products => {
        this.size = products.length;
      });
      this.msg = `${this.size} - POs loaded! for ${this.selectedVendor.name}`;
  ...
  onPickProduct(): void {
    const xSubscr = this.generatorForm.get('productid').valueChanges.subscribe(val => {
      ...
      const item: ReportItem = {name: this.selectedProduct.name, id: 0, price: this.selectedProduct.msrp, costprice: this.selectedProduct.costprice,
      ...
      if (this.items.length > 0) {
        this.msg = `Purchase Order: ${this.selectedProduct.id}`;
  ...
  onPickQuantity(): void {
    const xSubscr = this.generatorForm.get('quantityid').valueChanges.subscribe(val => {
      ...
      // tslint:disable-next-line:max-line-length
      const item: ReportItem = {name: this.selectedProduct.name, id: 0, price: this.selectedProduct.msrp,
        costprice: this.selectedProduct.costprice, productid: this.selectedProduct.id,
        po: 0, qty: val};
      ...
      if ( val === '0' ){
        ...
        this.msg = `details for PO ${this.selectedProduct.id}`;
      }
      else { // add entry
        this.items.push(item);
        this.selectedproducts.push(this.selectedProduct);
...
// map each product in the array and check whether or not it belongs to selected vendor
   ...
   */
  refreshDS(): void {
    this.vendors$.subscribe(products => {
      this.size = products.length;
    });
  } // refresh
